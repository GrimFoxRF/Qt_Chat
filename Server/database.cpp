...
bool DatabaseManager::openDatabase(const QString &databaseName)
{
    ...
    if (!database.open()) {
        ...
        //emit infoMessageDB("Ошибка при открытии базы данных: ");
        return false;
    }
    ...
    if (!database.tables().contains("users")) {
        if (!createUserTable()) {
            ...
            //emit infoMessageDB("Ошибка создания таблицы users");
            return false;
        }
    }
    ...
    if (!database.tables().contains("chat_history")) {
        if (!createChatHistoryTable()) {
            ...
            //emit infoMessageDB("Ошибка создания таблицы chat_history");
            return false;
        }
    }
...
bool DatabaseManager::executeQuery(const QString &query)
{
    ...
    if (!sqlQuery.exec(query)) {
        ...
        //emit infoMessageDB("Ошибка выполнения SQL-запроса ");
        return false;
    }
...
bool DatabaseManager::createAdminUser()
{
    ...
    // Создадим пользователя "admin" с паролем "admin"
